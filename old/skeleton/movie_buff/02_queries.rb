...
def vanity_projects
  ...
  Movie.select(:id, :title, :name).joins(:actors).where("movies.director_id = actors.id AND castings.ord = 1")
end
...
def most_supportive
  ...
  Actor
  .select("actors.id, actors.name, COUNT(castings.movie_id)")
  .joins(:castings)
  .group("actors.id")
  .having("castings.ord != 1")
  .order("COUNT(castings.movie_id)").limit(2)
end
